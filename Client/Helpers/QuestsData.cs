...
namespace ItemPurposeCheckmarks.Helpers
{
    internal static class QuestsData
    {
        ...
        public static void LoadData()
        {
            ...
            ParseData();
            _prereqCountCache.Clear();

            // Merged MoreCheckmarks data dimensions. These are independent of quests but share
            // the same profile-selected reload trigger.
            if (Settings.ShowBarter!.Value)
            {
                BarterHelper.LoadData();
            }

            if (Settings.ShowCraft!.Value)
            {
                CraftHelper.LoadData();
            }
        }
        ...
        private static void CheckUnreachableQuests()
        {
            ...
            for (int i = 0; i < _questsData!.Count; ++i)
            {
                ...
                for (int j = 0; j < startConditions.Count; ++j)
                {
                    ...
                    if (!Utils.IsValidMongoID(requirementIdRaw))
                    {
            ...
            QuestDependencyList = dependencyList;
        }

        // When unreachable quests are included there is no unreachable check, build the dependency list directly
        private static void BuildDependencyList()
        {
            Dictionary<MongoID, QuestRequirements> dependencyList = [];

            foreach (QuestJson quest2 in _questsData!)
            {
                if (quest2.Quest is not Quest quest || quest.Id is not MongoID questId)
                {
                    continue;
                }
                ...
                foreach (AvailableForStartCondition condition in startConditions)
                {
                    if (condition.ConditionType != "Quest" || condition.Target is not string targetRaw || !Utils.IsValidMongoID(targetRaw))
                    {
                        continue;
                    }

                    MongoID requirementId = targetRaw;
        ...
        /// <summary>
        /// Number of (transitive) prerequisite quests that are not yet completed for the given quest.
        /// </summary>
        public static int GetRemainingPrereqCount(MongoID questId, Profile profile)
        {
            if (_prereqCountCache.TryGetValue(questId, out int cached))
            {
                return cached;
            }

            HashSet<MongoID> allPrereqs = [];
            CollectPrereqIds(questId, allPrereqs, []);

            int remaining = allPrereqs.Count;

            if (profile.QuestsData is not null)
            {
                foreach (QuestDataClass questData in profile.QuestsData)
                {
                    if (questData.Template is not null && questData.Status == EQuestStatus.Success && allPrereqs.Contains(questData.Template.Id))
                    {
                        remaining--;
                    }
                }
            ...
            _prereqCountCache.Add(questId, remaining);
            return remaining;
        }

        private static void CollectPrereqIds(MongoID questId, HashSet<MongoID> result, HashSet<MongoID> stack)
        {
            if (!QuestDependencyList.TryGetValue(questId, out QuestRequirements requirements))
            {
                return;
            }
            ...
            foreach (QuestRequirements req in requirements.List)
            {
                if (result.Add(req.QuestId) && stack.Add(req.QuestId))
                {
                    CollectPrereqIds(req.QuestId, result, stack);
                }
            }
        }
        ...
        private static void ParseData()
        {
            ...
            for (int i = 0; i < _questsData!.Count; ++i)
            {
                ...
                for (int j = 0; j < finishConditions.Count; ++j)
                {
                    ...
                    List<MongoID> targets = [.. targetsRaw.Where(Utils.IsValidMongoID).Select(t => new MongoID(t))];
        ...
        public static void RemoveQuest(MongoID questId)
        {
            ...
            foreach (KeyValuePair<MongoID, QuestItems> item in questItems)
            {
                ...
                {
                    Plugin.LogSource?.LogError($"Attempted to remove non-existing quest item {item.Key} from quest {questId} data!");
                    continue;
                }
